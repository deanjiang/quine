...
/* Compare two files byte-for-byte; return 1 if identical */
...
int main(int argc, char **argv) {
    ...
    if (!strcmp(argv[1], "compress") && argc == 5) {
        ...
        printf("\nverifying...\n");
        r = quine_decompress(dir_a, patch_path, tmpdir);
        if (r) {
            fprintf(stderr, "  FAIL: decompress during verify: %s\n", quine_errmsg());
            rmtree(tmpdir);
            return 1;
        }
        if (dirs_equal(dir_b, tmpdir)) {
